...
generate_{{ worker }}_cert:
  cmd.run:
    - name: cfssl gencert -ca=ca.pem -ca-key=ca-key.pem -config=ca-config.json -hostname={{ worker }},{{ private_ip[worker][0] }},{{ public_ip[worker][0] }} -profile=kubernetes {{ worker }}-csr.json | cfssljson -bare {{ worker }}
    - cwd: /root
{% endfor %}
